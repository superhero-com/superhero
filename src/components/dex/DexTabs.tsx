...
const Tab = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
  <button
    onClick={onClick}
    className={`px-6 py-3 rounded-xl border transition-all duration-300 ease-in-out min-h-[48px] touch-manipulation uppercase tracking-wider font-semibold text-sm
  ...
  return (
    <div className="flex gap-3 mb-6 items-center flex-wrap justify-center md:gap-2 md:mb-5 sm:gap-1.5 sm:mb-4">
      ...
      <button
        ...
        className="px-6 py-3 rounded-xl border border-transparent transition-all duration-300 ease-in-out min-h-[48px] touch-manipulation uppercase tracking-wider font-semibold text-sm text-[var(--light-font-color)]
                   md:px-4 md:py-2.5 md:min-h-[44px] md:text-xs
                   sm:px-3 sm:py-2 sm:min-h-[40px] sm:text-[10px]
