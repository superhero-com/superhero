...
export default function FooterSection({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <footer className={`${compact ? 'border-t mt-3 py-3' : 'border-t mt-6 py-4 md:py-5 md:mt-8'}`} style={{ borderTopColor: 'var(--search-nav-border-color)' }}>
      <div className={`max-w-[min(1200px,100%)] mx-auto ${compact ? 'px-3 flex flex-col items-center gap-1.5' : 'px-4 flex gap-4 items-center'} md:flex-col md:gap-4 md:px-4 md:text-center sm:px-3 sm:gap-3`}>
      ...
      <div className={`max-w-[min(1200px,100%)] mx-auto ${compact ? 'px-3' : 'px-4'} mt-2 flex ${compact ? 'justify-center' : 'justify-end'} md:justify-center md:px-4 sm:px-3`}>
