...
const ShareModal = ({
  ...
  const resolvedTitle = title ?? t('common.modals.share.title');
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <button
        ...
        aria-label={t('common.modals.share.closeModal')}
        onClick={onClose}
      />
      <div className={cn(
        ...
        <div className="px-6 pb-4">
          <div className="mb-4">
            <div className="block text-sm font-medium text-white/60 mb-2">
              {t('common.modals.share.shareWithOthers')}
            </div>
            <CopyText
              value={shareUrl}
              className="w-full"
          ...
          <div className="mb-4">
            <div className="text-sm font-medium text-white/60 mb-3">
              {t('common.modals.share.shareOnSocialMedia')}
            </div>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(t('common.modals.share.checkOutToken'))}`)}
              ...
              >
                {t('common.modals.share.twitter')}
              </button>
              <button
                type="button"
                onClick={() => window.open(`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(t('common.modals.share.checkOutToken'))}`)}
              ...
              >
                {t('common.modals.share.telegram')}
              </button>
            </div>
          </div>
        </div>
