...
export function useTokenBalances({ assets, direction, aeAccount, ethAccount, sdk }: UseTokenBalancesProps) {
  ...
  const fetchBalances = useCallback(async () => {
    ...
    try {
      ...
      const fetchAeternityBalances = async () => {
        if (!currentAeAccount || !currentAeAccount.startsWith('ak_') || !currentSdk) return;
  ...
  useEffect(() => {
    console.log('[useTokenBalances] Effect triggered - fetching balances');
    fetchBalances();
  }, [assets, aeAccount, ethAccount, sdk, fetchBalances]);

  return { aeBalances, ethBalances, loading, refetch: fetchBalances };
}
