...
function PoolContent() {
  ...
  const rightRail = (
    <div className="hidden lg:block">
      <div id="liquidity-forms-section" className="flex flex-col gap-6">
  ...
  return (
    <Shell right={rightRail} containerClassName="max-w-[min(1200px,100%)] mx-auto">
      <div className="md:py-0 flex flex-col gap-6 md:gap-8 min-h-screen px-2 md:px-4">
        ...
        {/* Positions / activity as center content */}
          ...
          <div className="mb-6">
              ...
              <div className="flex gap-2">
                {activeAccount && (
                  <button
                    ...
                    disabled={loading}
                    className={`px-4 py-2 rounded-xl border border-white/10 bg-white/[0.02] text-white cursor-pointer text-xs font-semibold transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] backdrop-blur-[10px] flex items-center gap-2 ${
                      loading
            ...
            {loading && positions.length === 0 ? (
              ...
              <div className="text-center p-10 bg-white/[0.03] rounded-2xl border border-white/10 backdrop-blur-[10px]">
                ...
                <div className="text-base font-semibold mb-2 text-white">No liquidity positions found</div>
                <div className="text-sm text-white/60 mb-5 leading-relaxed">Start earning fees by providing liquidity to trading pairs</div>
                {!activeAccount && (
                  <ConnectWalletButton
              ...
              <div className="flex flex-col gap-3">
                {positions
                  .filter((position) => position?.pair?.address)
                  .map((position, index) => (
        ...
        {/* Recent Activity under main card */}
        <div className="mt-6">
          <RecentActivity />
        </div>
      </div>
