...
export default function PostForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`[PostForm] ${isPost ? 'Post' : 'Comment'} submitted`, decodedResult);
  ...
  if (placeholder) {
    ...
    currentPlaceholder = activeAccount ? PROMPTS[promptIndex] : 'Connect your wallet to start posting ✍️';
  } else {
    currentPlaceholder = activeAccount ? 'Write a comment...' : 'Connect your wallet to comment';
  }
          ...
          <div className="flex flex-col gap-3 pt-2.5 border-t border-white/12 relative md:flex-row md:items-center md:justify-between md:gap-3 md:pt-3">
            ...
            <div className="flex items-center justify-center w-full md:justify-end md:w-auto">
              {activeAccount ? (
                ...
                >
                  {isSubmitting ? (isPost ? 'Posting…' : 'Posting...') : (isPost ? 'Post' : 'Post Comment')}
                </AeButton>
              ) : (
                <ConnectWalletButton 
                  block 
