...
export function useSuperheroChainNames() {
  ...
  // On app load, fetch all known chain names from the backend cache in a
  // single request and merge them into the atom.
  useEffect(() => {
    let cancelled = false;

    const backendUrl = configs.networks.ae_mainnet.superheroBackendUrl.replace(/\/$/, '');
    fetch(`${backendUrl}/cache/chainNames`)
      .then((res) => res.json())
      .then((names) => {
        if (!cancelled && names && typeof names === 'object') {
          setChainNames((prev) => ({ ...prev, ...names }));
        }
      })
      .catch((e) => {
        console.warn('Failed to load chain names on app load', e);
      });
