...
function ratioFromPairReserves(pairReserves: Reserves[]): BigNumber {
  return pairReserves.reduce((ratio, [reserveA, reserveB]) => ratio.multipliedBy(BigNumber(reserveB).div(reserveA)), BigNumber(1));
}
...
export function ratioWithDecimals(ratio: NumberLike, { decimalsA, decimalsB }: { decimalsA: number; decimalsB: number }): BigNumber {
...
export function getPath(route: { token0: string; token1: string }[], tokenA: string): string[] {
  ...
  const [first, last] = (ordered as { token0: string; token1: string }[]).reduce<[string[], string]>(
    ([acc, prev], pair) => {
...
function getReceivedTokensForOnePair(reserveA: NumberLike, reserveB: NumberLike, amountA: NumberLike): BigNumber {
...
export function getReceivedTokensForPairReserves(pairReserves: Reserves[], amountA: NumberLike): BigNumber {
...
export function getPriceImpactForRoute(route: RoutePair[], tokenA: string, amountA: NumberLike): number {
